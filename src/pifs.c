...
static const int (*pifs_read_choisi)() = max_read;
static const int (*pifs_write_choisi)() = pifs_write_max;
...
{//ouvre un fichier
  ...
  if (options.retour_fonction != -1) info->fh = options.retour_fonction;
  return options.retour_fonction == -1 ? -errno : 0;
	 ...
	 */
	int pifs_read(const char *path, char *buf, size_t count, off_t offset,
                     struct fuse_file_info *info){
		     return pifs_read_choisi(path, buf,  count,  offset, info);
		 }
	 ...
	 * expected to reset the setuid and setgid bits.
      ...
      À moins que FUSE_CAP_HANDLE_KILLPRIV ne soit désactivé, cette méthode devrait réinitialiser les bits setuid et setgid.
	 */
	int pifs_write(const char *path, const char *buf, size_t count, off_t offset,
                     struct fuse_file_info *info){
              return pifs_write_choisi(path, buf, count,offset,info);
		  }
